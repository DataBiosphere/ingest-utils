...
object MonsterLibraryPlugin extends AutoPlugin {
  ...
  /** Hostname of our Google Artifact Registry. */
  val garBase = "artifactregistry://us-central1-maven.pkg.dev/dsp-artifact-registry/"
//
...
//  val ArtifactoryPasswordVar = "ARTIFACTORY_PASSWORD"
  ...
  def fullResolverPath(isSnapshot: Boolean): String = {
    ...
    s"$garBase/libs-$target-standard"
  }
...
//  }
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      publishTo := Some("google-artifact-registry" at fullResolverPath(isSnapshot.value))
    )
}
